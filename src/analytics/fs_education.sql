...
tb_pct_cursos AS (
    SELECT 
        *,
        1. * qtdeEps / qtdeTotalEps AS pctCursoCompleto
...
tb_pct_cursos_pivot AS (
    ...
    SELECT idUsuario, 
        ...
        sum(CASE WHEN pctCursoCompleto > 0 AND pctCursoCompleto < 1 THEN 1 ELSE 0 END) AS qtdeCursoIncompleto,
        sum(CASE WHEN pctCursoCompleto > 0 THEN 1 ELSE 0 END) AS qtdeCursosIniciados,
...
tb_join AS (
    SELECT 
        ...
        t1.qtdeCursoIncompleto,
        t1.qtdeCursosIniciados,
        t1.carreira,
        t1.coletaDados2024,
        t1.dsDatabricks2024,
        t1.dsPontos2024,
